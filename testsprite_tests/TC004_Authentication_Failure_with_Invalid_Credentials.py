...
async def run_test():
    ...
    try:
        ...
        # Verify an error message is displayed indicating authentication failure
        frame = context.pages[-1]
        error_elem = frame.locator('text=Invalid credentials.')
        assert await error_elem.is_visible()
          
        # Verify no redirection to Dashboard occurs
        assert 'dashboard' not in frame.url.lower()
