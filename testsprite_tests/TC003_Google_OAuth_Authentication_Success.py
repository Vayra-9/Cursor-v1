...
async def run_test():
    ...
    try:
        ...
        # Click the 'Continue with Google' button to initiate Google OAuth sign in.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input the email 'test@vayra.digital' into the email field to proceed with Google OAuth sign-in.
        frame = context.pages[-1]
        ...
        # Click the 'Try again' button to retry the Google OAuth sign-in or consider testing the redirect fallback method if retry fails.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div[2]/c-wiz/main/div[3]/div/div/div/div/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input the email 'test@vayra.digital' into the email field to proceed with Google OAuth sign-in retry.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div[2]/c-wiz/main/div[2]/div/div/div/form/span/section/div/div/div/div/div/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('test@vayra.digital')
