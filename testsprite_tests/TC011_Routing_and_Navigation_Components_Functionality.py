...
async def run_test():
    ...
    try:
        ...
        # Click 'Start Your Journey' to begin sign-in or sign-up process.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/section/div/div/div[2]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input email and password for test+vayra@demo.local and sign in.
        frame = context.pages[-1]
        ...
        # Click 'Back to Dashboard' to navigate to dashboard.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[3]/div/div/a[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Try to navigate directly to the dashboard URL or report the website issue if navigation fails.
        await page.goto('http://localhost:5174/dashboard', timeout=10000)
        

        # Input email and password for test+vayra@demo.local and sign in to test Starter plan.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/div[2]/form/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('test+vayra@demo.local')
        ...
        # Click 'Back to Dashboard' to navigate to dashboard and verify access and navigation menus.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[3]/div/div/a[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
