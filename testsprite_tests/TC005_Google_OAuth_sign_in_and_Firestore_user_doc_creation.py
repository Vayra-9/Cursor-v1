...
async def run_test():
    ...
    try:
        ...
        # Find a way to navigate to the sign-in page or directly go to /auth/sign-in if no navigation available.
        await page.goto('http://localhost:5174/auth/sign-in', timeout=10000)
        

        # Click the 'Continue with Google' button to start the OAuth workflow.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input the test Google account email 'test+vayra@demo.local' into the email field and click Next to proceed with OAuth.
        frame = context.pages[-1]
