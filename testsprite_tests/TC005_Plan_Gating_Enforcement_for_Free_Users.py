...
async def run_test():
    ...
    try:
        ...
        # Click on 'Get Started Free' or equivalent to initiate sign in as free-tier user.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/section[3]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input email and password for free-tier user and click Sign In button.
        frame = context.pages[-1]
        ...
        # Verify that premium feature content is blurred with a prompt to upgrade, then click an upgrade button to verify upgrade flow initiation.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[3]/div/div/a[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click the 'Get Started' button on the Starter plan to verify upgrade flow initiation.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div[2]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on a premium feature's upgrade prompt to verify redirection to the Upgrade page and clear upgrade flow.
        frame = context.pages[-1]
